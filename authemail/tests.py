...
class SignupTests(APITestCase):
    ...
    def test_signup_serializer_errors(self):
        error_dicts = [
            # Email required
            {
                "payload": {"email": "", "password": self.user_visitor_pw},
                "status_code": status.HTTP_400_BAD_REQUEST,
                "error": ("email", "This field may not be blank."),
            },
            # Password required
            {
                "payload": {"email": self.user_visitor_email, "password": ""},
                "status_code": status.HTTP_400_BAD_REQUEST,
                "error": ("password", "This field may not be blank."),
            },
            # Invalid email
            {
                "payload": {"email": "XXX", "password": self.user_visitor_pw},
                "status_code": status.HTTP_400_BAD_REQUEST,
        ...
        for error_dict in error_dicts:
            ...
            self.assertEqual(
                response.data[error_dict["error"][0]][0], error_dict["error"][1]
            )
    ...
    def test_signup_twice_then_email_verify(self):
        ...
        for i in range(0, num_signups):
            ...
            self.assertEqual(mail.outbox[i].subject, "Verify your email address")
...
class LoginTests(APITestCase):
    def setUp(self):
        ...
        self.user_verified_email = "user_verified@mail.com"
        self.user_verified_pw = "user_verified"
        self.user_verified = get_user_model().objects.create_user(
            self.user_verified_email, self.user_verified_pw
        )
        ...
        self.user_not_verified = get_user_model().objects.create_user(
            self.user_not_verified_email, "pw"
        )
        ...
        self.user_not_active = get_user_model().objects.create_user(
            self.user_not_active_email, self.user_not_active_pw
        )
    ...
    def test_login_serializer_errors(self):
        error_dicts = [
            # Email required
            {
                "payload": {"email": "", "password": self.user_verified_pw},
                "status_code": status.HTTP_400_BAD_REQUEST,
                "error": ("email", "This field may not be blank."),
            },
            # Password required
            {
                "payload": {"email": self.user_verified_email, "password": ""},
                "status_code": status.HTTP_400_BAD_REQUEST,
                "error": ("password", "This field may not be blank."),
            },
            # Invalid email
            {
                "payload": {"email": "XXX", "password": self.user_verified_pw},
                "status_code": status.HTTP_400_BAD_REQUEST,
                "error": ("email", "Enter a valid email address."),
            },
        ]
    ...
    def test_login_invalid_credentials(self):
        ...
        self.assertEqual(
            response.data["detail"], "Unable to login with provided credentials."
        )
...
class PasswordResetTests(APITestCase):
    ...
    def test_password_reset_user_not_verified_not_allowed(self):
        url = reverse("authemail-password-reset")
        payload = {"email": self.user_not_verified_email}
        response = self.client.post(url, payload)
    ...
    def test_password_reset_user_not_active_not_allowed(self):
        url = reverse("authemail-password-reset")
        payload = {"email": self.user_not_active_email}
        response = self.client.post(url, payload)
    ...
    def test_password_reset_verify_expired_code(self):
        ...
        password_reset_code = PasswordResetCode.objects.latest("code")
        password_reset_code.created_at += timedelta(
            days=-(PasswordResetCode.objects.get_expiry_period() + 1)
        )
...
class EmailChangeTests(APITestCase):
    def setUp(self):
        ...
        self.user_to_change = get_user_model().objects.create_user(
            self.user_to_change_email, "pw"
        )
    ...
    def test_email_change_verify_expired_code(self):
        ...
        email_change_code = EmailChangeCode.objects.latest("code")
        email_change_code.created_at += timedelta(
            days=-(EmailChangeCode.objects.get_expiry_period() + 1)
        )
...
class PasswordChangeTests(APITestCase):
    def setUp(self):
        ...
        self.user_to_change = get_user_model().objects.create_user(
            self.user_to_change_email, self.user_to_change_pw
        )
        self.user_to_change.is_verified = True
        self.user_to_change.save()
